...
#if defined(NDEBUG)
# error "Number7 cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1a22c18d80aebf4cc477974649e882fc4a62dbf55a268858ceae60d88dc8d9ef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Number7: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Number7 Signed Message:\n";
...
{
    // Number7: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 300 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 18967742 * COIN;
            }

    // Subsidy is cut in half every 420000 blocks
    nSubsidy >>= (nHeight / 420000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 14 * 60;
static const int64 nTargetSpacing = 7 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Number7: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Number7: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1454799127, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "RT 6 Feb 2016 Russian Su35 fighters to be on 24h alert at Latakia base";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 300 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042108a952003b87041202a88a03314ecc6073ad11ceb4c27226ba93655c900ce533c835a54231227d55108e6b50759bac5dcf06e527352e226eaf5ffba516b882") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1454799127;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1734121;
        ...
        assert(block.hashMerkleRoot == uint256("0xc2a3b9fae59151f822d3567531f03f64392e53fcf4ddba6f51dc278d718efb6c"));
...
unsigned char pchMessageStart[4] = { 0x37, 0xac, 0x48, 0x69 }; // Number7: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Number7Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Number7Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Number7Miner(CWallet *pwallet)
{
    printf("Number7Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("number7-miner");
    ...
    try { loop {
        ...
        printf("Running Number7Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Number7Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Number7Miner, pwallet));
}
